...
sendIF::sendIF(QObject *parent):QIODevice(parent)
{
    ...
    connect(&m_pTcpSocket, SIGNAL(error(QTcpSocket::SocketError)), this, SLOT(onSocketError(QTcpSocket::SocketError)));
}
...
{
    if(m_isTCP){
        m_pTcpSocket.connectToHost(m_portName,23);
        QIODevice::open(mode);
        return m_pTcpSocket.waitForConnected(3000);
    }else{
        QIODevice::open(mode);
